...
public class RefreshIntegrationTest extends AbstractIntegrationTest {
    ...
    public void testFlushAll() throws InterruptedException, ExecutionException, TimeoutException, IOException {
        ...
        long totalRefreshesBefore = statsResponse.getTotal().getRefresh().getTotal();

        Refresh refresh = new Refresh.Builder().build();
        JestResult result = client.execute(refresh);
        assertTrue(result.getErrorMessage(), result.isSucceeded());

        statsResponseFeature = client().admin().indices().stats(
                new IndicesStatsRequest().clear().flush(true).refresh(true));
        statsResponse = statsResponseFeature.get(10, TimeUnit.SECONDS);
        assertNotNull(statsResponse);

        assertEquals(totalRefreshesBefore + statsResponse.getTotalShards(), statsResponse.getTotal().getRefresh().getTotal());
    }
    ...
    public void testFlushSpecificIndices() throws InterruptedException, ExecutionException, TimeoutException, IOException {
        ...
        Map<String, IndexStats> indicesStatsBefore = statsResponse.getIndices();
        ...
        assertEquals(indicesStatsBefore.get("i_flush_4").getTotal().getRefresh().getTotal() + stats4.getShards().length, stats4.getTotal().getRefresh().getTotal());

        IndexStats stats5 = statsResponse.getIndex("i_flush_5");
        assertEquals(indicesStatsBefore.get("i_flush_5").getTotal().getRefresh().getTotal(), stats5.getTotal().getRefresh().getTotal());

        IndexStats stats6 = statsResponse.getIndex("i_flush_6");
        assertEquals(indicesStatsBefore.get("i_flush_6").getTotal().getRefresh().getTotal() + stats6.getShards().length, stats6.getTotal().getRefresh().getTotal());
    }
